...
0. **按简历打穿（主线）**：`文档/03_按简历模块打穿学习手册.md`——只在本全景学习，对齐 `面试模拟/实习模拟.md`。
1. **先读** `文档/00_项目地图.md`——整条 Agent 链、工具联动、评测、数据来源。
2. 读 `文档/02_同步_并发_异步定稿.md`——目标第3档；并行 vs 异步；学习优先级。
3. 分模块精读：`文档/简历第一条_…`～`简历第四条_…`；讲解在 `app/`，验证在 `demos/`。
4. 再读差别文档与灰度评估，分清「原文内核」和「ops 补丁」。
5. 学习对错记录在仓库根目录 `00_学习进度与复盘笔记.md`。
